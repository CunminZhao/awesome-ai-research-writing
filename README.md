...
# Input
[在此处粘贴你的中文草稿];将 LaTeX 源码直接输出在 Markdown 代码块中，不要进行任何格式预览，确保我可以完整复制源码。将 LaTeX 源码直接输出在 Markdown 代码块中，不要进行任何格式预览，确保我可以完整复制源码。将 LaTeX 源码直接输出在 Markdown 代码块中，不要进行任何格式预览，确保我可以完整复制源码。 流形M，encoder E和生成器D都用\mathcal{}手写体
````
